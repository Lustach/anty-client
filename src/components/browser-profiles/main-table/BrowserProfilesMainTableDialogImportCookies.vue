<template>
  ...
  >
    <v-card>
      ...
      >
        <v-toolbar-title>
          {{ $t('browserProfiles.importCookies') }}
        </v-toolbar-title>
      ...
      <v-card-text>
        <v-row>
          ...
          >
            <v-alert
              v-if="Array.isArray(cookies) && cookies.length !== 0"
              color="success"
              text
            >
              {{ $t('common.validCookies', {type: cookiesType}) + " " + cookies.length }}
            </v-alert>
            <v-alert
              v-else
              color="error"
      ...
      <v-card-actions>
        ...
        <v-btn
          ...
          color="primary"
          :loading="loading"
          :disabled="cookies === null || !Array.isArray(cookies) || cookies.length === 0"
          @click="importCookies"
        >
          {{ $t('common.import') }}
        </v-btn>
...
export default {
  name: 'BrowserProfilesMainTableDialogImportCookies',
  components: {
    Drag
  },
  data () {
  ...
  watch: {
    cookiesInput: {
      handler () {
        this.parseCookies(this.cookiesInput)
      }
    }
  },
  ...
  methods: {
    ...
    dropHandler (event) {
      ...
      reader.onload = function () {
        _this.cookiesInput = reader.result
      }
    },

    close () {
      this.$store.dispatch('browserProfiles/closeDialog', 'importCookies')
    },

    async importCookies () {
      try {
        this.loading = true
        const response = await this.syncApi.post(`/?actionType=importCookies&browserProfileId=${this.browserProfile.id}`, { cookies: this.cookies })
        if (response.status === 200) {
          this.$store.dispatch('main/alert', { color: 'success', message: this.$t('common.done') })
    ...
    parseCookies (cookiesString) {
      ...
      if (this.isJsonString(cookiesString)) {
        ...
        if (cookies[0].creation) {
          this.cookies = this.parseBasCookies(cookies)
        }
      ...
      this.cookies = this.parseNetscapeCookies(cookiesString)
    },
    ...
    handleInputUpload () {
      ...
      for (const file of this.$refs.inputUpload.files) {
        ...
        reader.onload = function () {
          _this.cookiesInput = reader.result
          // _this.parseCookies(reader.result)
        }
      }
    },
...
<style  >
.import-cookie .v-text-field__slot {
  flex: none !important;
  width: 101% !important;
